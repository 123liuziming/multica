...
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, allow_ask_user_question
`
...
SELECT id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, allow_ask_user_question FROM agent
WHERE id = $1
`
...
UPDATE agent SET
    ...
    allow_ask_user_question = COALESCE($16, allow_ask_user_question),
    updated_at = now()
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, allow_ask_user_question
`
...
type UpdateAgentParams struct {
	...
	McpConfig            []byte      `json:"mcp_config"`
	Model                pgtype.Text `json:"model"`
	AllowAskUserQuestion pgtype.Bool `json:"allow_ask_user_question"`
}
