...
const listWorkspacesWithAoneSetting = `-- name: ListWorkspacesWithAoneSetting :many
SELECT id, name, slug, description, settings, created_at, updated_at, context, repos, issue_prefix, issue_counter FROM workspace
WHERE settings->>'aone_project_id' IS NOT NULL
`

func (q *Queries) ListWorkspacesWithAoneSetting(ctx context.Context) ([]Workspace, error) {
	rows, err := q.db.Query(ctx, listWorkspacesWithAoneSetting)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
